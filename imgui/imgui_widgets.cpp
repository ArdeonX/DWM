...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    char name[16];
    ImFormatString(name, IM_ARRAYSIZE(name), "c_%02d", g.BeginPopupStack.Size); // Recycle windows based on depth
...
bool ImGui::DragFloatRange2(const char* label, float* v_current_min, float* v_current_max, float v_speed, float v_min, float v_max, const char* format, const char* format_max, float power)
{
    ...
    bool value_changed = DragScalar(" ", ImGuiDataType_Float, v_current_min, v_speed, &min, &max, format, power);
    PopItemWidth();
    SameLine(0, g.Style.ItemInnerSpacing.x);
    ...
    if (min == max) { min = FLT_MAX; max = -FLT_MAX; } // Lock edit
    value_changed |= DragScalar(" ", ImGuiDataType_Float, v_current_max, v_speed, &min, &max, format_max ? format_max : format, power);
    PopItemWidth();
    SameLine(0, g.Style.ItemInnerSpacing.x);
...
bool ImGui::DragIntRange2(const char* label, int* v_current_min, int* v_current_max, float v_speed, int v_min, int v_max, const char* format, const char* format_max)
{
    ...
    bool value_changed = DragInt(" ", v_current_min, v_speed, min, max, format);
    PopItemWidth();
    SameLine(0, g.Style.ItemInnerSpacing.x);
    ...
    if (min == max) { min = INT_MAX; max = INT_MIN; } // Lock edit
    value_changed |= DragInt(" ", v_current_max, v_speed, min, max, format_max ? format_max : format);
    PopItemWidth();
    SameLine(0, g.Style.ItemInnerSpacing.x);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    {
        IM_ASSERT(state != NULL);
        IM_ASSERT(io.KeyMods == GetMergedKeyModFlags() && ""); // We rarely do this check, but if anything let's do it here.
...
ImGuiID ImGui::GetColumnsID(const char* str_id, int columns_count)
{
    ...
    ImGuiID id = window->GetID(str_id ? str_id : "c");
    PopID();
