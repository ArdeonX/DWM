...
static ID3D10Device1*           g_pd3dDevice1 = NULL;
...
bool    ImGui_ImplDX10_CreateDeviceObjects()
{
    ...
    static D3DCompile_t PD3DCompile = NULL;
    static HMODULE hDll = NULL;
    if (!PD3DCompile)
    {
        for (int i = 47; i > 30 && !PD3DCompile; i--)
        {
            char dll_name[20];
            sprintf_s(dll_name, "d3dcompiler_%02d.dll", i);
            if (hDll = LoadLibraryA(dll_name))
                PD3DCompile = (D3DCompile_t)GetProcAddress(hDll, "D3DCompile");
        }
        if (!PD3DCompile)
            return false;
    }

    char vs_str[20] = { 0 }, ps_str[20] = { 0 };
    D3D10_FEATURE_LEVEL1 feature_levels;
    feature_levels = g_pd3dDevice1->GetFeatureLevel();
    if (feature_levels == D3D10_FEATURE_LEVEL_9_1)
    {
        strcpy_s(vs_str, "vs_4_0_level_9_1");
        strcpy_s(ps_str, "ps_4_0_level_9_1");
    }
    else if (feature_levels == D3D10_FEATURE_LEVEL_9_2)
    {
        strcpy_s(vs_str, "vs_4_0_level_9_2");
        strcpy_s(ps_str, "ps_4_0_level_9_2");
    }
    else if (feature_levels == D3D10_FEATURE_LEVEL_9_3)
    {
        strcpy_s(vs_str, "vs_4_0_level_9_3");
        strcpy_s(ps_str, "ps_4_0_level_9_3");
    }
    else
    {
        strcpy_s(vs_str, "vs_4_0");
        strcpy_s(ps_str, "ps_4_0");
    }
    // Create the vertex shader
    {
        ...
        ID3DBlob* eervertexShaderBlob;
        if (FAILED(PD3DCompile(vertexShader, strlen(vertexShader), NULL, NULL, NULL, "main",vs_str, 0, 0, &vertexShaderBlob, &eervertexShaderBlob)))
            return false; // NB: Pass ID3DBlob* pErrorBlob to D3DCompile() to get error showing in (const char*)pErrorBlob->GetBufferPointer(). Make sure to Release() the blob!
        if (g_pd3dDevice->CreateVertexShader((DWORD*)vertexShaderBlob->GetBufferPointer(), vertexShaderBlob->GetBufferSize(), &g_pVertexShader) != S_OK)
        {
    ...
    // Create the pixel shader
    {
        ...
        if (FAILED(PD3DCompile(pixelShader, strlen(pixelShader), NULL, NULL, NULL, "main", ps_str, 0, 0, &pixelShaderBlob, NULL)))
            return false; // NB: Pass ID3DBlob* pErrorBlob to D3DCompile() to get error showing in (const char*)pErrorBlob->GetBufferPointer(). Make sure to Release() the blob!
...
bool    ImGui_ImplDX10_Init(ID3D10Device* device, ID3D10Device1* device1)
{
    // Setup back-end capabilities flags
    ImGuiIO& io = ImGui::GetIO();
    io.BackendRendererName = "xml10";
